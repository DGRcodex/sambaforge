...
async function injectTextIntoStory(storyXmlPath, newText) {
  ...
  },
  {
    ...
    content: 'El dominio de la automatización requiere práctica resolviendo problemas reales. A continuación, se detallan los conceptos aplicados en laboratorios.',
    rules: [
      'Laboratorio 1 (Script Resiliente): Todo script debe iniciar apagando la interacción (NEVER_INTERACT), envolver la lógica en un try/catch, y cerrar el documento en el finally (SaveOptions.NO). Mitigar el Overset Text requiere un bucle seguro.',
      'Laboratorio 2 (Root Cause Analysis): Errores comunes incluyen "Off-by-One" (i < array.length, NO i <= array.length), bloqueos de interfaz por alert() en headless, y Memory Leaks por no cerrar documentos.',
...
function buildPublicationHeadless(templatePath, articleData, outputPdfPath) {  
  ...
  },
  {
    ...
    content: 'En procesamiento industrial (33 millones de páginas/mes), la verdadera escalabilidad se logra mutando archivos XML y encolando tareas de manera resiliente.',
    rules: [
      'IDML Mutation: Un IDML es un .zip. El texto editorial se guarda en "Stories/Story_X.xml". Un backend Node.js puede inyectar datos (parseando a JSON, mutando el nodo <Content>) en milisegundos sin abrir InDesign.',
...
async function injectTextIntoStory(storyXmlPath, newText) {
  ...
  },
  {
    ...
    content: 'Mastering automation requires practice solving real problems. Below are the main concepts applied in labs.',
    rules: [
      ...
      'Lab 2 (Root Cause Analysis): Common errors include "Off-by-One" (i < array.length, NOT <=), interface locks due to alert() in headless mode, and Memory Leaks from not closing documents.',
...
function buildPublicationHeadless(templatePath, articleData, outputPdfPath) {  
  ...
  },
  {
    ...
    content: 'At 33M pages/month scale, true scalability comes from direct XML mutation and resilient queuing.',
    rules: [
